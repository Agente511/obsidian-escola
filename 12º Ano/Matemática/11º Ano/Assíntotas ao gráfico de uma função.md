...
>   Identificar os **pontos $a$** em que a **função pode ser descontínua**.
>2. Calcular, para os valores de $a$ identificados, **$\underset{x \rightarrow a^-} \lim f(x)$ e $\underset{x \rightarrow a^+} \lim f(x)$**.
>3. Se **$\underset{x \rightarrow a} \lim f(x) =(+ \text{ ou } -)\infty$**, conclui-se que **$x=a$ é uma assíntota vertical ao gráfico da função $f$**.
